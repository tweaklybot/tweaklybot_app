...
fun ViewerScreen(
    ...
    LaunchedEffect(lastTapTime) {
        delay(3000)
        if (System.currentTimeMillis() - lastTapTime >= 2900) barsVisible = false
    }

    fun onTap() { barsVisible = true; lastTapTime = System.currentTimeMillis() }
    ...
    Box(Modifier.fillMaxSize().background(Color.Black).pointerInput(Unit) {
        ...
        when {
            ...
            state.item != null -> ZoomableImage(state.item!!.uri) { onTap() }
        }
        ...
        // ── Bottom bar ─────────────────────────────────────────────────────
        AnimatedVisibility(
            barsVisible && !state.isLoading && state.item != null,
            ...
            Box(
                ...
                Row(Modifier.fillMaxWidth(), Arrangement.SpaceEvenly) {
                    ViewerAction(Icons.Default.Share, "Поделиться") { vm.share(context) }
                    ...
                    if (state.item?.mediaType == MediaType.PHOTO) {
                        if (state.isOcrRunning) SmallLoader()
                        else ViewerAction(Icons.Default.TextFields, "OCR") { vm.runOcr(context) }
                        if (state.isQrRunning) SmallLoader()
                        else ViewerAction(Icons.Default.QrCodeScanner, "QR") { vm.scanQr(context) }
                    }
                }
            }
        }
    ...
    if (state.showDeleteDialog) {
        AlertDialog(
            ...
            text = { Text("Файл будет удалён с устройства. Это действие нельзя отменить.") },
            confirmButton = {
                TextButton({ vm.hideDeleteDialog(); onBack() }) {
    ...
    if (state.showInfoDialog) {
        state.item?.let { item ->
            AlertDialog(
                onDismissRequest = { vm.hideInfoDialog() },
                ...
                text = {
                    Column(Modifier.verticalScroll(rememberScrollState()),
                        ...
                        item.remotePath?.let { InfoRow("Путь в репозитории", it) }
                    }
                },
    ...
    state.ocrResult?.let { text ->
        AlertDialog(
            ...
            confirmButton = {
                TextButton({
                    clipboard.setText(AnnotatedString(text)); vm.clearOcr()
                }) { Text("Скопировать") }
            },
    ...
    state.qrResult?.let { text ->
        ...
        AlertDialog(
            onDismissRequest = { vm.clearQr() },
            icon = { Icon(Icons.Default.QrCode, null) },
            title = { Text("QR-код") },
            text = { SelectionContainer { Text(text) } },
            confirmButton = {
                if (isUrl) {
                    TextButton({ uriLauncher.openUri(text); vm.clearQr() }) { Text("Открыть ссылку") }
                } else {
                    TextButton({ clipboard.setText(AnnotatedString(text)); vm.clearQr() }) { Text("Скопировать") }
                }
            },
...
@Composable
private fun ZoomableImage(uri: String, onTap: () -> Unit) {
...
private fun fmtSize(b: Long) = when {
    ...
    b >= 1_000 -> "%.1f КБ".format(b / 1_000.0)
    else -> "$b Б"
}

private fun fmtDate(ts: Long): String {
    if (ts == 0L) return "Неизвестно"
